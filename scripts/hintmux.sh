...
tmux send-keys -t $SESSION "cd $WS && source install/setup.bash && ros2 launch stonefish_sim simulation.launch.py scenario:=tacc rendering_quality:=low" C-m
...
#tmux split-window -h -t $SESSION
#tmux send-keys -t $SESSION "cd $WS && source install/setup.bash && ros2 launch operation_mode_manager operation_mode_manager.launch.py" C-m
