...
class AdaptiveLOSGuidance
{
    public:
        ...
        void update_angles(const LOS::Point& prev_point, const LOS::Point& next_point);
        LOS::CrossTrackError calculate_crosstrack_error(const LOS::Point& prev_point, const LOS::Point& current_position);
        double calculate_psi_d(const double& y_e);
        double calculate_theta_d(const double& z_e);
        void update_adaptive_estimates(const LOS::CrossTrackError& crosstrack_error);
