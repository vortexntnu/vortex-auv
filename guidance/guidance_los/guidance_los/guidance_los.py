...
class GuidanceNode(Node):
    ...
    def compute_3d_los(self, e_y, e_z, pi_p, gamma, current_depth):
        ...
        theta_d = np.clip(gamma + math.atan2(-e_z, self.delta_vertical), -max_pitch, max_pitch)
    ...
    def timer_callback(self):
        ...
        e_z = current_z - (ref_z + (dz/math.hypot(dx, dy)) * e_x)  # Vertical error

        # Compute 3D LOS guidance commands
        heading_d, pitch_d = self.compute_3d_los(e_y, e_z, pi_p, gamma, current_z)
        ...
        self.get_logger().info(f"Position: x={current_x:.2f}, y={current_y:.2f}, z={current_z:.2f}")
        self.get_logger().info(f"Target: x={target_x:.2f}, y={target_y:.2f}, z={target_z:.2f}")
        self.get_logger().info(f"Commands: surge={surge_velocity:.2f}, pitch={pitch_d:.2f}, yaw={heading_d:.2f}")
        self.get_logger().info(f"Errors: cross-track={e_y:.2f}, vertical={e_z:.2f}, heading={e_psi:.2f}")
