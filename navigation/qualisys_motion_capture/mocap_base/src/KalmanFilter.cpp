...
KalmanFilter::KalmanFilter()
    : attitude(Quaterniond::Identity()), position(Vector3d::Zero()),
      angular_vel(Vector3d::Zero()), linear_vel(Vector3d::Zero()),
      state_cov(Matrix12d::Identity()), input_cov(Matrix12d::Identity()),
      measurement_cov(Matrix6d::Identity()), filter_status(INIT_POSE),
      last_time_stamp(0.0), msg_interval(0.01), proc_jacob(Matrix12d::Zero()),
      meas_jacob(Matrix6_12d::Zero()), proc_noise_jacob(Matrix12d::Zero()),
...
bool KalmanFilter::init(const Matrix12d &u_cov, const Matrix6d &m_cov,
                        const int &freq) {
  bool is_valid = true;
  ...
  if (m_sigmas(5) < 1e-10) {
    is_valid = false;
    ROS_ERROR(
        "Measurement Cov is close to singular (least singlar value:%f < 1e-7)",
        m_sigmas(5));
  } else {
...
bool KalmanFilter::prepareInitialCondition(const double &curr_time_stamp,
...
void KalmanFilter::prediction(const double &curr_time_stamp) {
  ...
  // cout << "Process: " << endl;
  // cout << "att: " <<
  //  Vector4d(attitude.w(), attitude.x(), attitude.y(),
  //  attitude.z()).transpose() << endl;
...
void KalmanFilter::update(const Eigen::Quaterniond &m_attitude,
  ...
  // cout << "Error: " << endl;
  // cout << "dat: " << Vector4d(re_q.w(), re_q.x(), re_q.y(),
  // re_q.z()).transpose() << endl; cout << "dpo: " << re_r.transpose() << endl;
