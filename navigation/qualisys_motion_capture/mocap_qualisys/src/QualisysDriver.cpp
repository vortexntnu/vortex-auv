...
bool QualisysDriver::init() {
  ...
  ROS_INFO_STREAM("Connecting to QTM server at: " << server_address << ":"
                                                  << base_port);
  ...
  if (!port_protocol.Connect((char *)server_address.data(), base_port,
                             udp_port_ptr, major_protocol_version,
  ...
  if (frame_rate < system_frequency && frame_rate > 0) {
    ...
    if (frame_rate > system_frequency) {
      ROS_WARN("Requested capture rate %i larger than current system capture "
               "rate %i.",
               frame_rate, system_frequency);
    }
    frame_rate = system_frequency;
  }
  bDataAvailable = port_protocol.StreamFrames(stream_rate_mode,
                                              frame_rate,      // nRateArg
...
void QualisysDriver::disconnect() {
  ROS_INFO_STREAM("Disconnected from the QTM server at " << server_address
                                                         << ":" << base_port);
...
void QualisysDriver::handleFrame() {
  ...
  if (start_time_local_ == 0) {
    ...
    ROS_WARN("Dropped old frame from time %fdropped, previous frame was from "
             "time %f",
...
void QualisysDriver::handleSubject(int sub_idx) {
  ...
  if (publish_tf &&
    ...
    ROS_DEBUG("%s rot matrix:\n%f,\t%f,\t%f\n%f,\t%f,\t%f\n%f,\t%f,\t%f\n",
              subject_name.c_str(), rot_array[0], rot_array[1], rot_array[2],
              rot_array[3], rot_array[4], rot_array[5], rot_array[6],
              rot_array[7], rot_array[8]);
    ROS_DEBUG("Position\nx: %f,\ty: %f\tz: %f", x / 1000.0, y / 1000.0,
              z / 1000.0);
    ROS_DEBUG("Quaternion rotation\nx: %f,\ty: %f,\tz: %f,\tw: %f,\t",
              att_tf.getX(), att_tf.getY(), att_tf.getZ(), att_tf.getW());
    tf::StampedTransform stamped_transform =
        tf::StampedTransform(tf::Transform(att_tf, pos_tf), ros::Time(time),
