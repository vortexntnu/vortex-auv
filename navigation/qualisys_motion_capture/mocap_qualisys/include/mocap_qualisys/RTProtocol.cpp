...
bool CRTProtocol::Calibrate(const bool refine, SCalibration &calibrationResult,
                            int timeout) {
  char pResponseStr[256];
...
CRTProtocol::GetComponents(const std::string componentsString) {
  std::vector<std::pair<unsigned int, std::string>>
      components; // Vector containing pair (component, option string).
...
bool CRTProtocol::GetComponent(std::string componentStr,
                               unsigned int &component, std::string &option) {
  // Make string lower case.
  std::transform(componentStr.begin(), componentStr.end(), componentStr.begin(),
                 ::tolower);
  option = "";
...
bool ReadXmlFov(std::string name, CMarkup &oXML,
                CRTProtocol::SCalibrationFov &fov) {
...
bool CRTProtocol::ReceiveCalibrationSettings(int timeout) {
  ...
  if (eType != CRTPacket::PacketXML) {
    if (eType == CRTPacket::PacketError) {
      ...
      sprintf(maErrorStr,
              "GetParameters Calibration returned wrong packet type. Got type "
              "%d expected type 2.",
              eType);
    }
    return false;
  ...
  try {
    ...
    settings.result_min_max_diff =
        std::stod(oXML.GetChildAttrib("min-max-diff"));
    ...
    while (oXML.FindChildElem("camera")) {
      ...
      std::transform(calibratedStr.begin(), calibratedStr.end(),
                     calibratedStr.begin(), ::tolower);
      ...
      camera.intrinsic.sensor_min_u =
          std::stod(oXML.GetChildAttrib("sensorMinU"));
      camera.intrinsic.sensor_max_u =
          std::stod(oXML.GetChildAttrib("sensorMaxU"));
      camera.intrinsic.sensor_min_v =
          std::stod(oXML.GetChildAttrib("sensorMinV"));
      camera.intrinsic.sensor_max_v =
          std::stod(oXML.GetChildAttrib("sensorMaxV"));
      camera.intrinsic.focal_length_u =
          std::stod(oXML.GetChildAttrib("focalLengthU"));
      camera.intrinsic.focal_length_v =
          std::stod(oXML.GetChildAttrib("focalLengthV"));
      camera.intrinsic.center_point_u =
          std::stod(oXML.GetChildAttrib("centerPointU"));
      camera.intrinsic.center_point_v =
          std::stod(oXML.GetChildAttrib("centerPointV"));
      camera.intrinsic.skew = std::stod(oXML.GetChildAttrib("skew"));
      camera.intrinsic.radial_distortion_1 =
          std::stod(oXML.GetChildAttrib("radialDistortion1"));
      camera.intrinsic.radial_distortion_2 =
          std::stod(oXML.GetChildAttrib("radialDistortion2"));
      camera.intrinsic.radial_distortion_3 =
          std::stod(oXML.GetChildAttrib("radialDistortion3"));
      camera.intrinsic.tangental_distortion_1 =
          std::stod(oXML.GetChildAttrib("tangentalDistortion1"));
      camera.intrinsic.tangental_distortion_2 =
          std::stod(oXML.GetChildAttrib("tangentalDistortion2"));
      oXML.OutOfElem(); // camera
...
void CRTProtocol::GetCalibrationSettings(
    SCalibration &calibrationSettings) const {
...
std::vector<CRTProtocol::SSettingsGeneralCamera>
CRTProtocol::GetDevices() const {
...
const char *
CRTProtocol::GetGazeVectorName(unsigned int nGazeVectorIndex) const {
...
unsigned int
CRTProtocol::GetForcePlateChannelCount(unsigned int nPlateIndex) const {
  if (nPlateIndex < msForceSettings.vsForcePlates.size()) {
