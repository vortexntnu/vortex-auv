...
UnderwaterOdom::UnderwaterOdom() {
  ...
  // Subscribers
  fluid_pressure_sub_ = nh_.subscribe("/auv/pressure", 1,
                                      &UnderwaterOdom::pressureCallback, this);
  dvl_twist_sub_ =
      nh_.subscribe("/auv/dvl_twist", 1, &UnderwaterOdom::dvlCallback, this);
...
void UnderwaterOdom::pressureCallback(const sensor_msgs::FluidPressure &msg) {
  ...
  const float depth_meters =
      gauge_pressure / (water_density * earth_gravitation);

  // update odom pose, ENU
  odom.pose.pose.position.z = -depth_meters;
}

void UnderwaterOdom::dvlCallback(
