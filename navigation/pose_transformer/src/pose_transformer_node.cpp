...
class PoseTransformerNode : public rclcpp::Node {
   public:
    PoseTransformerNode() : Node("pose_transformer_node") {
        ...
        pose_sub_.subscribe(this, "/aruco_board_pose_camera",
        ...
        tf2_filter_ = std::make_shared<
            ...
            pose_sub_, *tf2_buffer_, "odom", 100,
            this->get_node_logging_interface(),
            this->get_node_clock_interface());

        tf2_filter_->registerCallback(
            std::bind(&PoseTransformerNode::poseCallback, this, _1));

        // Publisher for the transformed PoseStamped message
        transformed_pose_pub_ =
            this->create_publisher<geometry_msgs::msg::PoseStamped>(
                "/aruco_board_pose_odom", 10);

        RCLCPP_INFO(this->get_logger(),
                    "PoseTransformerNode has been started.");
    }
    ...
    void poseCallback(
        ...
        try {
            tf2_buffer_->transform(
                *pose_msg, transformed_pose, "odom",
