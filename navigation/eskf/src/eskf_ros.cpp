...
void ESKFNode::set_parameters() {
    ...
    this->declare_parameter<std::vector<double>>(
        "diag_Q_std");  // gyroscope bias noise
    ...
    Q.diagonal() << sq(diag_Q_std[0]), sq(diag_Q_std[1]),
        sq(diag_Q_std[2]),  // acceleration noise
        sq(diag_Q_std[3]), sq(diag_Q_std[4]),
        sq(diag_Q_std[5]),  // gyroscope noise
        sq(diag_Q_std[6]), sq(diag_Q_std[7]),
        sq(diag_Q_std[8]),  // acceleration bias noise
        sq(diag_Q_std[9]), sq(diag_Q_std[10]),
        sq(diag_Q_std[11]);  // gyroscope bias noise
    eskf_params_.Q = Q;
