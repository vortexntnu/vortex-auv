...
import numpy as np
from eskf_python_class import Measurement, StateQuat
...
def simulate_eskf():
    ...
    Q = np.diag(
        [
            (0.13**2),
            (0.13**2),
            (0.13**2),  # Adjusted Accelerometer noise
            (0.13**2),
            (0.13**2),
            (0.13**2),  # Adjusted Gyroscope noise
            0.0001,
            0.0001,
            0.0001,  # Adjusted Acceleration bias random walk
            0.0001,
            0.0001,
            0.0001,  # Adjusted Gyro bias random walk
        ]
    )

    # Create filter object
    eskf = ESKF(Q, P0, true_state_init, 1e-13, 1e-13, dt)
    ...
    dvl_data.aiding_covariance = np.diag(
        [(0.01) ** 2, (0.01) ** 2, (0.01) ** 2]
    )  # Adjusted DVL aiding covariance
    ...
    for step in range(num_steps):
        ...
        imu_data.acceleration = (
            (new_state.velocity - prev_velocity) / dt
        ) + np.random.normal(0, 0.13, 3)
        imu_data.angular_velocity = new_state.angular_velocity + np.random.normal(
            0, 0.13, 3
        )
...
for i in range(3):
    ...
    ax_pos.plot(
        time,
        true_positions[:, i],
        label=f"True Pos {axis_labels_pos[i]}",
        color=f"C{i}",
        linestyle='-',
    )
    ax_pos.plot(
        time,
        est_positions[:, i],
        label=f"Est Pos {axis_labels_pos[i]}",
        color=f"C{i}",
        linestyle='--',
    )
...
for i in range(3):
    ...
    ax_vel.plot(
        time,
        true_velocities[:, i],
        label=f"True Vel {axis_labels_vel[i]}",
        color=f"C{i}",
        linestyle='-',
    )
    ax_vel.plot(
        time,
        est_velocities[:, i],
        label=f"Est Vel {axis_labels_vel[i]}",
        color=f"C{i}",
        linestyle='--',
    )
...
for i in range(3):
    ...
    ax_ori.plot(
        time,
        true_orientations[:, i],
        label=f"True Ori {axis_labels_ori[i]}",
        color=f"C{i}",
        linestyle='-',
    )
    ax_ori.plot(
        time,
        est_orientations[:, i],
        label=f"Est Ori {axis_labels_ori[i]}",
        color=f"C{i}",
        linestyle='--',
    )
