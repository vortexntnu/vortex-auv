...
def create_statequat(
    ...
    if noise_angle < 1e-8:
        ...
        noise_quat = np.concatenate(
            ([np.cos(noise_angle / 2)], np.sin(noise_angle / 2) * noise_axis)
        )
...
def test_state_statistics():
    ...
    for _ in range(num_states):
        ...
        state_list.append(
            create_statequat(
                base_vector, pos_noise, ori_noise, vel_noise, ang_vel_noise
            )
        )
...
def create_statequat(base_vector, pos_noise, ori_noise, vel_noise, ang_vel_noise):
    ...
    state.okid_params.fill(
        np.concatenate(
            (
                np.array([0.0, 0.0, 0.3, 0.0, 0.0, 3.3, 0.0, 0.0, 3.3]),
...
def run_ukf_simulation():
    ...
    Q[3:6, 3:6] = (
        np.eye(3) * 0.001
    )  # for orientation error (represented with Euler angles)
    ...
    true_state = create_statequat(
        base_vector, np.zeros(3), np.zeros(3), np.zeros(3), np.zeros(3)
    )
    true_state.covariance = P0.copy()

    filter_state = create_statequat(
        base_vector, np.zeros(3), np.zeros(3), np.zeros(3), np.zeros(3)
    )
        ...
        if i % 5 == 0:
            try:
                ...
                filter_state.covariance += (
                    np.eye(filter_state.covariance.shape[0]) * 1e-6
                )
