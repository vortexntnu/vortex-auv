...
class AcousticsDataRecordNode(Node):
    ...
    def __init__(self):
        ...
        self.subscriberFilterResponse = self.create_subscription(
            Int32MultiArray, '/acoustics/filter_response',
            self.filter_response_callback, 5)
        self.filterResponseData = array.array("i", [0] * DSPDataSize)

        self.subscriberFFT = self.create_subscription(Int32MultiArray,
                                                      '/acoustics/fft',
                                                      self.fft_callback, 5)
        ...
        self.subscriberTDOAResponse = self.create_subscription(
            Float32MultiArray, '/acoustics/time_difference_of_arrival',
            self.tdoa_callback, 5)
        self.TDOAData = array.array("f", [0.0] * TDOADataSize)

        self.subscriberPositionResponse = self.create_subscription(
            Float32MultiArray, '/acoustics/position', self.position_callback,
            5)
