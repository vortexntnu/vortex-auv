...
class AcousticsInterfaceNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._fft_publisher = self.create_publisher(Int32MultiArray,
                                                    '/acoustics/fft', 5)
        self._peak_publisher = self.create_publisher(Int32MultiArray,
                                                     '/acoustics/peaks', 5)
        ...
        self._timer_data_publisher = self.create_timer(timer_period,
                                                       self.data_publisher)
    ...
    def data_publisher(self) -> None:
        ...
        self._hydrophone_1_publisher.publish(
            Int32MultiArray(
                data=TeensyCommunicationUDP.acoustics_data["HYDROPHONE_1"]))
        self._hydrophone_2_publisher.publish(
            Int32MultiArray(
                data=TeensyCommunicationUDP.acoustics_data["HYDROPHONE_2"]))
        self._hydrophone_3_publisher.publish(
            Int32MultiArray(
                data=TeensyCommunicationUDP.acoustics_data["HYDROPHONE_3"]))
        self._hydrophone_4_publisher.publish(
            Int32MultiArray(
                data=TeensyCommunicationUDP.acoustics_data["HYDROPHONE_4"]))
        self._hydrophone_5_publisher.publish(
            Int32MultiArray(
                data=TeensyCommunicationUDP.acoustics_data["HYDROPHONE_5"]))

        self._filter_response_publisher.publish(
            Int32MultiArray(data=TeensyCommunicationUDP.
        ...
        self._peak_publisher.publish(
            Int32MultiArray(
                data=TeensyCommunicationUDP.acoustics_data["PEAK"]))

        self._tdoa_publisher.publish(
            Float32MultiArray(
                data=TeensyCommunicationUDP.acoustics_data["TDOA"]))
        self._position_publisher.publish(
            Float32MultiArray(
                data=TeensyCommunicationUDP.acoustics_data["LOCATION"]))
