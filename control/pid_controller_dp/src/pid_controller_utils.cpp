...
Eigen::Vector7d error_eta(const Eigen::Vector7d &eta, const Eigen::Vector7d &eta_d) {
  ...
  Eigen::Vector4d q_error((q_eta_d(0) * q_eta(0)) - (q_eta_d(1) * q_eta(1) + q_eta_d(2) * q_eta(2) + q_eta_d(3) * q_eta(3)),
                             (q_eta_d(0) * q_eta(1)) + (q_eta(0) * q_eta_d(1)) + ((-q_eta_d(3) * q_eta(2)) + (q_eta_d(2) * q_eta(3))),
                             (q_eta_d(0) * q_eta(2)) + (q_eta(0) * q_eta_d(2)) + ((q_eta_d(3) * q_eta(1)) + (-q_eta_d(1) * q_eta(3))),
                             (q_eta_d(0) * q_eta(3)) + (q_eta(0) * q_eta_d(3)) + ((-q_eta_d(2) * q_eta(1)) + (q_eta_d(1) * q_eta(2))));
...
Eigen::Matrix6x7d calculate_J_sudo_inv(const Eigen::Vector7d &eta) {
  ...
  // Set a tolerance to handle singular values
  double tolerance = 1e-6;
  Eigen::VectorXd singularValuesInv = svd.singularValues().unaryExpr([&](double x) {
      return (std::abs(x) > tolerance) ? 1.0 / x : 0.0;
  });
...
Eigen::Vector6d limit_input(const Eigen::Vector6d &input) {
  ...
  std::cout << "Limited Input: " << limited_input.transpose() << std::endl;
