...
DPAdaptBacksControllerCore::DPAdaptBacksControllerCore(
    ...
    : time_step_(config.time_step),
      controller_(
          std::make_unique<DPAdaptBacksController>(config.controller_params)),
...
Eigen::Vector6d DPAdaptBacksControllerCore::tick(
    const vortex::utils::types::Pose& pose,
    const vortex::utils::types::Pose& reference_pose,
    const vortex::utils::types::Twist& twist) {
    return controller_->calculate_tau(pose, reference_pose, twist);
}
...
vortex::utils::types::Mode DPAdaptBacksControllerCore::operation_mode()
    const noexcept {
