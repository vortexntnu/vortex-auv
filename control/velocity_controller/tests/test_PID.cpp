...
TEST_F(PID_test, reset_controller) {
    ...
    PID.set_parameters(0, 1, 0, 1);
    PID.calculate_thrust(0);
    PID.reset_controller();
...
TEST_F(PID_test, P) {
    PID.set_parameters(1, 0, 0, 1);
    EXPECT_NEAR(PID.calculate_thrust(1), 1, delta);
    EXPECT_NEAR(PID.calculate_thrust(2), 2, delta);
    PID.set_parameters(1.2, 0, 0, 1);
    EXPECT_NEAR(PID.calculate_thrust(-2.2), -2.64, delta);
    EXPECT_NEAR(PID.calculate_thrust(-1.5), -1.8, delta);
}
TEST_F(PID_test, I) {
    PID.set_parameters(0, 1.1, 0, 1);
    PID.calculate_thrust(1);
    EXPECT_NEAR(PID.get_output(), 0, delta);
    PID.calculate_thrust(1);
    EXPECT_NEAR(PID.get_output(), 1.1, delta);
    PID.calculate_thrust(-1);
    EXPECT_NEAR(PID.get_output(), 2.2, delta);
    EXPECT_NEAR(PID.calculate_thrust(0), 1.1, delta);
    PID.set_output_limits(-101, 101);
    PID.reset_controller();
    PID.set_parameters(1, 1, 0, 10);
    EXPECT_NEAR(PID.calculate_thrust(1000), 101, delta);
    PID.set_parameters(1, 1, 0, 1);
    EXPECT_NEAR(PID.calculate_thrust(0), 0, delta);
    PID.reset_controller();
    EXPECT_NEAR(PID.calculate_thrust(-10000), -101, delta);
    PID.calculate_thrust(-50);
    EXPECT_NEAR(PID.calculate_thrust(1), -49, delta);
}
TEST_F(PID_test, D) {}
TEST_F(PID_test, illegal_inputs) {
    double temp = PID.get_output();
    PID.set_parameters(1, 1, 0, 0);
