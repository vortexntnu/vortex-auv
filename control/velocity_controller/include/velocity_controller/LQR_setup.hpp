...
struct LQRsolveResult{
    ...
    Eigen::MatrixXd P;
    int INFO=0;
    LQRsolveResult(Eigen::MatrixXd K=Eigen::MatrixXd::Zero(),Eigen::MatrixXd
P=Eigen::MatrixXd::Zero(), int INFO=0):K(K),P(P),INFO(INFO) {};
};*/
class LQRController {
   public:
    LQRController();
   ...
   private:
    ...
    Eigen::Vector<double, 8> update_error(const Guidance_data& error,
                                          const State& state);
...
// std::vector<std::vector<double>> matrix3d_to_vector2d(const Eigen::Matrix3d
// &mat); Eigen::Matrix3d vector2d_to_matrix3d(const
