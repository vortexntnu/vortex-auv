...
Velocity_node::Velocity_node() : rclcpp_lifecycle::LifecycleNode("velocity_controller_lifecycle"), lqr_controller(), pub_QoS(10),  sub_QoS(10)
{
  ...
  //LQR
  if(!lqr_controller.set_matrices(Q,R,inertia_matrix,max_force,dampening_matrix_low)||!lqr_controller.set_interval(static_cast<double>(publish_rate)/1000)){
    controller_type=1;
    RCLCPP_INFO(this->get_logger(),"Switching to PID");
  };
  //Automaticly start in activate if auto_start is true
  if(auto_start){
...
void Velocity_node::calc_thrust()
{
  ...
  default:{
    RCLCPP_ERROR(this->get_logger(),"Unknown controller set, switching to PID");
    controller_type=1;
    calc_thrust();
    return;
  }
...
void Velocity_node::get_new_parameters(){
  ...
  this->declare_parameters<bool>("Settings", {{"auto_start", false}, {"reset_on_new_ref", true}, {"anti_overshoot", true},{"odometry_dropout_guard", true}});
  auto settings = this->get_parameters({"Settings.auto_start", "Settings.reset_on_new_ref", "Settings.anti_overshoot", "Settings.odometry_dropout_guard"});
