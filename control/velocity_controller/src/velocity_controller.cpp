...
Velocity_node::Velocity_node()
    ...
    publisher_reference =
        create_publisher<std_msgs::msg::Float64MultiArray>("/reference", 2);
    ...
    subscriber_killswitch = this->create_subscription<std_msgs::msg::Bool>(
        topic_killswitch, 10,
        std::bind(&Velocity_node::killswitch_callback, this,
                  std::placeholders::_1));

    // Timer

    timer_calculation =
        this->create_wall_timer(std::chrono::milliseconds(calculation_rate),
...
void Velocity_node::publish_thrust() {
    ...
    // RCLCPP_DEBUG(this->get_logger(),"Publishing thrust:
    // %.3f",thrust_out.wrench.force.x);
}
...
void Velocity_node::calc_thrust() {
    switch (controller_type) {
        case 1: {
            ...
            PID_surge.calculate_thrust(guidance_values.surge,
                                       current_state.surge,
                                       calculation_rate / 1000.0);
            PID_pitch.calculate_thrust(guidance_values.pitch,
                                       current_state.pitch,
                                       calculation_rate / 1000.0);
            PID_yaw.calculate_thrust(guidance_values.yaw, current_state.yaw,
                                     calculation_rate / 1000.0);
        ...
        case 2: {
            ...
            Eigen::Vector3d u =
                lqr_controller.calculate_lqr_u(current_state, guidance_values);
    ...
    msg.data = {guidance_values.surge, guidance_values.pitch,
                guidance_values.yaw};
...
void Velocity_node::guidance_callback(
    ...
    // RCLCPP_DEBUG(this->get_logger(), "Guidance received: surge=%.3f
    // pitch=%.3f yaw=%.3f",
    //              guidance_values.surge, guidance_values.pitch,
    //              guidance_values.yaw);
    return;
}
...
void Velocity_node::killswitch_callback(
    const std_msgs::msg::Bool::SharedPtr msg_ptr) {
    ...
    if (msg_ptr->data == true) {
        ...
        RCLCPP_INFO(
            this->get_logger(),
            "Killswitch activated, reference and current state set to zero");
    }
    return;
}
...
void Velocity_node::get_new_parameters() {
    ...
    // this->topic_twist =
    // this->get_parameter("topics.twist_topic").as_string();
    ...
    this->topic_killswitch =
        this->get_parameter("topics.killswitch_topic").as_string();
    ...
    this->lqr_parameters.q_surge =
        this->get_parameter("LQR_params.q_surge").as_double();
    this->lqr_parameters.q_pitch =
        this->get_parameter("LQR_params.q_pitch").as_double();
    this->lqr_parameters.q_yaw =
        this->get_parameter("LQR_params.q_yaw").as_double();
    this->lqr_parameters.r_surge =
        this->get_parameter("LQR_params.r_surge").as_double();
    this->lqr_parameters.r_pitch =
        this->get_parameter("LQR_params.r_pitch").as_double();
    this->lqr_parameters.r_yaw =
        this->get_parameter("LQR_params.r_yaw").as_double();
    this->lqr_parameters.i_surge =
        this->get_parameter("LQR_params.i_surge").as_double();
    this->lqr_parameters.i_pitch =
        this->get_parameter("LQR_params.i_pitch").as_double();
    this->lqr_parameters.i_yaw =
        this->get_parameter("LQR_params.i_yaw").as_double();
    this->lqr_parameters.i_weight =
        this->get_parameter("LQR_params.i_weight").as_double();
...
/*
geometry_msgs::msg::WrenchStamped operator-(const
geometry_msgs::msg::WrenchStamped & a, const geometry_msgs::msg::WrenchStamped &
b)
{
...
Guidance_data& Guidance_data::operator=(const std_msgs::msg::Float64MultiArray&
msg)
{
  if (msg.data.size()>=3){
    surge=msg.data[0];
    ...
    yaw=msg.data[2];
      ...
      //throw std::runtime_error("Guidance message too short, needs at least 3
values"); RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Guidance message too
short, needs at least 3 values");
  }
  return *this;
}
*/
