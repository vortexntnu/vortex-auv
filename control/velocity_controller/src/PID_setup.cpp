...
/*
PID_controller::PID_controller( double Kp, double Ki, double Kd, double
max_output, double min_output, double dt):Kp_(Kp), Ki_(Ki), Kd_(Kd),
...
bool PID_controller::calculate_thrust(double error) {
    ...
    // P + I + D
    output =
        Kp_ * error + Ki_ * integral + Kd_ * (error - previous_error) / dt_;
