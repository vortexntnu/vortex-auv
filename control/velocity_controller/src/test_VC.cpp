...
test_VC::test_VC() : Node("test_VC_node"), current_state(0, 2, 2) {
    ...
    publisher_guidance = this->create_publisher<vortex_msgs::msg::LOSGuidance>(
        topic_guidance, 10);
    publisher_odom =
        this->create_publisher<nav_msgs::msg::Odometry>(topic_odom, 10);
    ...
    reference_msg.surge = 0.2;
    reference_msg.pitch = 0.3;
    reference_msg.yaw = 0.0;  // Surge, pitch, yaw
}
...
void test_VC::send_state() {
    ...
    odom_msg.pose.pose.orientation =
        euler_angle_to_quaternion(0.0, current_state.pitch, current_state.yaw);
    ...
    // RCLCPP_INFO(this->get_logger(), "Published state: '%f'",
    // current_state.surge);
    return;
    // RCLCPP_INFO(this->get_logger(), "Published state: '%f'",
    // current_state.pitch); RCLCPP_INFO(this->get_logger(), "Published state:
    // '%f'", current_state.yaw);
}
