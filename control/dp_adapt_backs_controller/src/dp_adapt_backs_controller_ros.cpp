...
DPAdaptBacksControllerNode::DPAdaptBacksControllerNode() : Node("dp_adapt_backs_controller_node") {
    ...
    std::string dp_reference_topic = this->get_parameter("dp_reference_topic").as_string();
    guidance_sub_ =
    ...
    std::string software_kill_switch_topic = this->get_parameter("software_kill_switch_topic").as_string();
    ...
    std::string software_operation_mode_topic = this->get_parameter("software_operation_mode_topic").as_string();
    ...
    tau_pub_ = this->create_publisher<geometry_msgs::msg::Wrench>(control_topic, 10);
...
void DPAdaptBacksControllerNode::pose_callback(
    ...
    eta_.ori = quat_to_euler(msg->pose.pose.orientation.w, msg->pose.pose.orientation.x,
...
void DPAdaptBacksControllerNode::twist_callback(
    ...
    nu_.angular_speed << msg->twist.twist.angular.x, msg->twist.twist.angular.y, msg->twist.twist.angular.z;
...
void DPAdaptBacksControllerNode::publish_tau() {
    ...
    dp_types::Vector6d tau = dp_adapt_backs_controller_.calculate_tau(eta_, eta_d_, nu_);   
...
void DPAdaptBacksControllerNode::set_adap_params() {
    this->declare_parameter<std::vector<double>>(
        "adap_param", {0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8});
    this->declare_parameter<std::vector<double>>(
