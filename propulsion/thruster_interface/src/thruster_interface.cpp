...
int ThrusterInterface::init_can() {
    can::CanStatus status = can_.init(can_interface_name_);

    if (status != can::CanStatus::OK) {
    ...
    status = can_.start_async_receive(
        [this](const struct canfd_frame& frame, can::CanStatus rx_status) {
...
std::vector<std::uint16_t> ThrusterInterface::interpolate_forces_to_pwm(
    const Eigen::Ref<const Eigen::VectorXd>& thruster_forces)
{
    std::vector<std::uint16_t> pwm(
        static_cast<std::size_t>(thruster_forces.size()));

    for (Eigen::Index i = 0; i < thruster_forces.size(); ++i) {
        const double force_in_kg = to_kg(thruster_forces(i));

        pwm[static_cast<std::size_t>(i)] = force_to_pwm(force_in_kg);
    }
...
std::uint16_t ThrusterInterface::calc_poly(double force,
...
std::optional<std::vector<std::uint16_t>> ThrusterInterface::drive_thrusters(
    const Eigen::Ref<const Eigen::VectorXd>& thruster_forces) {
    const Eigen::Index expected_size =
        static_cast<Eigen::Index>(thruster_parameters_.size());

    if (thruster_forces.size() < expected_size) {
        return std::nullopt;
    }

    Eigen::VectorXd mapped_forces(expected_size);

    for (std::size_t i = 0; i < thruster_parameters_.size(); ++i) {
        const auto& param = thruster_parameters_[i];
        const Eigen::Index idx = static_cast<Eigen::Index>(param.mapping);

        if (idx < 0 || idx >= thruster_forces.size()) {
            return std::nullopt;
        }

        mapped_forces(static_cast<Eigen::Index>(i)) =
            thruster_forces(idx) * param.direction;
    }
...
int ThrusterInterface::send_data_to_escs(
    ...
    if (!can_.is_initialized()) {
        return static_cast<int>(can::CanStatus::ERR_NOT_INITIALIZED);
    }
...
void ThrusterInterface::handle_can_frame(const struct canfd_frame& frame,
                                         can::CanStatus status) {
    if (status != can::CanStatus::OK) {
        return;
    }
...
void ThrusterInterface::set_fault_event_callback(FaultEventCallback callback) {
    fault_event_callback_ = std::move(callback);
}

void ThrusterInterface::set_pgood_event_callback(PGoodEventCallback callback) {
