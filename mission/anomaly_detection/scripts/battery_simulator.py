...
class BatterySimulator:
    def __init__(self):
        ...
        self.initial_battery_level = rospy.get_param(
            "~initial_battery_level", 100
        )  # percentage
        self.error_battery_level = rospy.get_param(
            "~error_battery_level", 20
        )  # percentage
        self.warn_battery_level = rospy.get_param(
        ...
        self.battery_step = (
            float(self.initial_battery_level) / self.rate / self.battery_runtime
        )
        ...
        # topics
        self.battery_level_pub = rospy.Publisher(
            "/auv/battery_level", Float32, queue_size=1
        )
        self.diag_pub = rospy.Publisher(
            "/auv/diagnostics", DiagnosticArray, queue_size=1
        )

        # services
        # manually set battery level
        rospy.Service(
            "~set_battery_level", SetBatteryLevel, self.SetBatteryLevelHandler
        )

        # create a dynamic reconfigure server and set a callback function
        self.battery_dyn_reconfigure = Server(
