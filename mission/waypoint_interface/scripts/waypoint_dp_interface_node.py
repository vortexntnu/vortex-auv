...
class WaypointDPInterface:
    def __init__(self):
        ...
        # Subscribers
        rospy.Subscriber("/move_base_simple/goal", PoseStamped, self.nav_goal_cb, queue_size=1)
        rospy.Subscriber("/move_base_simple/goal/z", Float32, self.nav_goal_z_cb, queue_size=1)
    ...
    def nav_goal_to_dp_setpoint(self, enable_z=False):
        ...
        try:
            self.tf_listener.waitForTransform('odom', self.nav_goal.header.frame_id, rospy.Time(), rospy.Duration(4.0))
            self.pose_target = self.tf_listener.transformPose('odom', self.nav_goal)
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            pass

        self.dp_position = [self.pose_target.pose.position.x, self.pose_target.pose.position.y, self.pose_target.pose.position.z]
        ...
        quaternion = (
            self.pose_target.pose.orientation.x,
        ...
        self.dp_client.wait_for_server()
        rospy.set_param("/setpoint/position", [float(self.dp_position[0]),float(self.dp_position[1]),float(self.dp_position[2])])
