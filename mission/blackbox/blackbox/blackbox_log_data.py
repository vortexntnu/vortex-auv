...
class BlackBoxLogData:
    ...
    def manage_csv_files(self, max_file_age_in_days=1, max_size_kb=3_000_000): #adjust the max size before you start deleting old files (1 000 000 kb = 1 000 mb = 1 gb)
        ...
        csv_files = [f for f in os.listdir(self.blackbox_data_directory) if f.endswith('.csv') and f.startswith('blackbox_data_') and pattern.match(f)]
        ...
        while total_size_kb > max_size_kb:
            ...
            csv_files_sorted = sorted(csv_files, key=lambda x: datetime.strptime(pattern.match(x).group(1), '%Y-%m-%d_%H:%M:%S'))
