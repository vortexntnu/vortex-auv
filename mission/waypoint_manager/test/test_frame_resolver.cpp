...
TEST(FrameResolver, WorldIsUnchanged) {
    ...
    const Pose out = resolve_pose(target, OffsetFrame::WORLD, start);
...
TEST(FrameResolver, BodyRelativeWithVehicleFacingEast) {
    ...
    const Pose forward =
        resolve_pose(make_pose(2.0, 0.0, 0.0, 0, 0, 0), OffsetFrame::BODY_RELATIVE, start);
    ...
    const Pose starboard =
        resolve_pose(make_pose(0.0, 1.0, 0.0, 0, 0, 0), OffsetFrame::BODY_RELATIVE, start);
    ...
    const Pose turned = resolve_pose(make_pose(0.0, 0.0, 0.0, 0, 0, M_PI / 6),
                                     OffsetFrame::BODY_RELATIVE, start);
...
TEST(FrameResolver, BodyRelativeWithRolledAndPitchedVehicle) {
    ...
    const Pose out = resolve_pose(offset, OffsetFrame::BODY_RELATIVE, start);
...
TEST(FrameResolver, WorldRelativeIgnoresVehicleHeadingForPosition) {
    ...
    const Pose up =
        resolve_pose(make_pose(0.0, 0.0, -1.0, 0, 0, 0), OffsetFrame::WORLD_RELATIVE, start);
    EXPECT_NEAR(up.x, 1.0, 1e-12);
    EXPECT_NEAR(up.y, 2.0, 1e-12);
    EXPECT_NEAR(up.z, 2.0, 1e-12);

    const Pose north =
        resolve_pose(make_pose(5.0, 0.0, 0.0, 0, 0, 0), OffsetFrame::WORLD_RELATIVE, start);
...
TEST(FrameResolver, WorldRelativeRotatesAboutOdomAxes) {
    ...
    const Pose out = resolve_pose(make_pose(0.0, 0.0, 0.0, 0, 0, 0.5),
                                  OffsetFrame::WORLD_RELATIVE, start);
