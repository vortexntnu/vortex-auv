...
def distance_between_points(p1, p2, convergence_height=0.0):
    return math.dist([p1.x, p1.y, p1.z + convergence_height], [p2.x, p2.y, p2.z])

# Check if new estimate is far enough away from old estimate to justify updating DP setpoint
def send_new_goal(coeff_a, coeff_b, object_pos,
                  current_goal_pos, own_pos):
    error = distance_between_points(object_pos,
                                    current_goal_pos)
