...
class DockingStandby(smach.State):
    ...
    def execute(self, userdata):
        ...
        while not rospy.is_shutdown():
            ...
            if 0.02 < distance_between_points(object_pos, self.docking.dp_client.goal.x_ref.position, self.convergence_height) or not self.docking.task_manager_client.was_enabled:
                self.docking.dp_client.set_acceptance_margins([self.margin,self.margin,self.margin,0.0,0.0,10.0])
            ...
            if not self.is_logged:
                ...
                rospy.loginfo(f"STARTING DOCKING CONVERGE TO HEIGHT {self.convergence_height}!")
                rospy.sleep(rospy.Duration(1))
                self.is_logged = True

            if self.docking.dp_client.has_reached_goal() and self.docking.task_manager_client.was_enabled:
                rospy.loginfo(f"{rospy.get_name()}: Converged {self.convergence_height}m above docking point!")
                return "succeeded"
