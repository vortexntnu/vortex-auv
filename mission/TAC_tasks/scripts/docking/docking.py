...
def within_acceptance_margins(self):
    ...
    max_error = 0.2
    error = np.sqrt(pow(self.odom.pose.pose.position.x - self.object.objectPose.pose.position.x,2)+pow(self.odom.pose.pose.position.y - self.object.objectPose.pose.position.y,2))
    if(error < max_error):
        return True
    return False
...
class DockingSearch(smach.State):
    def __init__(self):
        ...
        print("DockingSearch init start")
    ...
    def execute(self):
        # goal = DpGoal()
        print("DockingSearch execute start")
        rate = rospy.Rate(1)
        rate.sleep()
        ...
        print("DockingSearch execute end")
        return "succeeded"
...
class DockingExecute(smach.State):
    def __init__(self):
        ...
        print("DockingExecute init start")
    ...
    def execute(self):
        ...
        print("DockingExecute execute start")
        ...
        #     self.dp_client.cancel_all_goals()
        #     return "preempted"
        print("DockingExecute execute end")
...
class DockingStandby(smach.State):
    ...
    def execute(self):
        ...
        # goal.x_ref = undocking_pose
        # self.dp_client.send_goal(goal)
        print("DockingStandby_execute_start")
        rate = rospy.Rate(1)
        rate.sleep()
        ...
        print("DockingStandby_execute_end")
        return "succeded"
