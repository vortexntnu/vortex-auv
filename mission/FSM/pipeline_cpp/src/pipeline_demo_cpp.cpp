...
class FindPipelineState : public yasmin_ros::ActionState<FindDock> {
   public:
    ...
    std::string response_handler(
        ...
        blackboard->set<PoseStamped>("pipeline_start_pose",
                                     response->dock_pose);
...
class GoToStartPipelineState
    : public yasmin_ros::ActionState<NavigateWaypoints> {
   public:
    GoToStartPipelineState()
        : yasmin_ros::ActionState<NavigateWaypoints>(
              ...
              std::bind(&GoToStartPipelineState::response_handler,
                        this,
                        _1,
                        _2),
              std::bind(&GoToStartPipelineState::print_feedback,
                        this,
                        _1,
                        _2)) {};
    ...
    NavigateWaypoints::Goal create_goal_handler(
        ...
        waypoints.push_back(
            blackboard->get<PoseStamped>("pipeline_start_pose"));
        // Print the pipeline start pose
        fprintf(stderr, "Pipeline start pose: x = %f, y = %f, z = %f\n",
                waypoints[0].pose.position.x, waypoints[0].pose.position.y,
                waypoints[0].pose.position.z);
        fprintf(
            stderr,
            "Pipeline start pose orientation: w = %f, x = %f, y = %f, z = %f\n",
            waypoints[0].pose.orientation.w, waypoints[0].pose.orientation.x,
            waypoints[0].pose.orientation.y, waypoints[0].pose.orientation.z);
   ...
   public:
    ...
    NavigateWaypoints::Goal create_goal_handler(
        ...
        goal.waypoints.push_back(
            blackboard->get<PoseStamped>("aruco_waypoints"));
        return goal;
    }
...
int main(int argc, char* argv[]) {
    ...
    // add states
    sm->add_state(
        "FIND_PIPELINE", std::make_shared<FindPipelineState>(),
        {
    ...
    sm->add_state("GO_TO_START_OF_PIPELINE",
                  std::make_shared<GoToStartPipelineState>(),
                  {
    ...
    blackboard->set<std::vector<PoseStamped>>("aruco_waypoints",
                                              aruco_waypoints);
