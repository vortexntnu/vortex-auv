...
PoseActionServerNode::PoseActionServerNode() : Node("pose_action_server_node") {
    ...
    auto pose_sub_topic = this->declare_parameter<std::string>("pose_sub_topic");
    ...
    pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        pose_sub_topic, qos, std::bind(&PoseActionServerNode::pose_callback, this, _1));

    action_server_ = rclcpp_action::create_server<vortex_msgs::action::FilteredPose>(
        this, action_name,
...
void PoseActionServerNode::handleAccepted(const std::shared_ptr<GoalHandleFilteredPose> goal_handle)
{
...
void PoseActionServerNode::pose_callback(
    ...
    if (!is_executing_action_) return;
    
    pose_queue_.push_back(*pose_msg);

    geometry_msgs::msg::PoseStamped feedback_msg = *pose_msg;
    auto feedback = std::make_shared<vortex_msgs::action::FilteredPose::Feedback>();
