...
class VirtualTarget:
    ...
    def update_states(self, heading_mode, point=[0, 0]):
        ...
        if heading_mode == "point_dependent_heading":
            ...
            dot_kappa_o = lambda dot_s: 1 / (np.cos(chi_p - psi_t) * np.cos(
                gamma_p) * dot_s)
        ...
        self.nu_t = lambda dot_s: np.array([
            np.cos(chi_p - psi_t) * np.cos(gamma_p) * dot_s
             ...
             ] + [-np.sin(gamma_p) * dot_s] + [0] + [0] + [dot_psi_t(dot_s)])
    ...
    def optimize_along_track_speed(self, eta, nu, dot_eta_c, t):
        ...
        f = lambda x: -x
        g_1 = lambda x: -(np.dot(
            self.control_allocation_system.pseudo_inv_input_matrix, tau_c(x)) -
    ...
    def simulate_path_variables(self, t):
        ...
        if self.varpi > 1:
            if not self.path_index == len(self.path.path) - 1:
                self.path_index += 1
                rest = self.varpi - 1
                self.varpi = (rest * self.path.length[self.path_index - 1] /
    ...
    def generate_reference_trajectories(self,
                                        eta,
                                        nu,
                                        t,
                                        heading_mode,
...
if __name__ == "__main__":
    ...
    virtual_target = VirtualTarget(path, auv_model, actuator_model,
                                   dp_control_system)
