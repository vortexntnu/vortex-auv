...
class LocalPIDNode:
    ...
    def __init__(self):
        ...
        control_bandwidths       = [0.6, 1, 0.6, 0.6, 0.6, 0.6]   # Tune the PID control bandwidths for [surge, sway, heave, roll, pitch, yaw]
        ...
        self.Kp, self.Kd, self.Ki = self.pole_placement(control_bandwidths, relative_damping_ratios)

        self.perception_sub = rospy.Subscriber('/pfps/error_img', Point, self.local_error_cb)
        ...
        tf_lookup_init = self.__tfBuffer.lookup_transform(self.parent_frame, self.self.child_frame, rospy.Time(), rospy.Duration(5))
        quat = np.array([tf_lookup_init.transform.rotation.x,
    ...
    def pole_placement(self, control_bandwidths, relative_damping_ratios):
        ...
        for damp,i in enumerate(relative_damping_ratios):
            cursed_factor = 1 / (np.sqrt(1 - 2*damp**2 + np.sqrt(4*damp**4 - 4*damp**2 + 2)))
    ...
    def local_error_cb(self, msg):
        ...
        pos = np.array([tf_lookup_baselink.transform.translation.x, 
                        tf_lookup_baselink.transform.translation.y, 
                        tf_lookup_baselink.transform.translation.z])

        quat = np.array([tf_lookup_baselink.transform.rotation.x,
