...
class LocalPIDNode:
    ...
    def __init__(self):
        ...
        self.perception_sub = rospy.Subscriber("/pfps/error_img", Point,
                                               self.local_error_cb)
        self.tau_pub = rospy.Publisher("/pfps/local_control",
                                       Wrench,
                                       queue_size=1)
    ...
    def pole_placement(self, control_bandwidths, relative_damping_ratios):
        ...
        for damp, i in enumerate(relative_damping_ratios):
            cursed_factor = 1 / (np.sqrt(
                1 - 2 * damp**2 + np.sqrt(4 * damp**4 - 4 * damp**2 + 2)))
