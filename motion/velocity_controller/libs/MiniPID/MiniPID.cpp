...
void MiniPID::setI(double i) {
   ...
   * results in the output change due to the I term constant during the
   * transition.
   *
   */
}
...
void MiniPID::setMaxIOutput(double maximum) {
   ...
   * calculations against the max error are far more common than changing the I
   * term or Izone.
   */
  maxIOutput = maximum;
  if (I != 0) {
 ...
 * @param setpoint
 */
void MiniPID::setSetpoint(double setpoint) { this->setpoint = setpoint; }
 ...
 * @return calculated output value for driving the actual to the target
 */
double MiniPID::getOutput() { return getOutput(lastActual, setpoint); }
 ...
 * @return calculated output value for driving the actual to the target
 */
double MiniPID::getOutput(double actual) { return getOutput(actual, setpoint); }
 ...
 * @param range
 */
void MiniPID::setSetpointRange(double range) { setpointRange = range; }
