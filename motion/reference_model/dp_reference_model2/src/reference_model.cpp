...
void ReferenceModel::calculate_smooth(Eigen::Vector7d x_ref) {
  ...
  for (int i = 0; i < 7; i++) {
    eta_dot_d(i) =
        std::max(std::min(eta_dot_d(i), saturation_vec(i)), -saturation_vec(i));
  }
...
// x_ref << position_setpoint, orientation_setpoint.w(),
// orientation_setpoint.vec(); std::cout << x_ref << std::endl;
//   while (ros::ok()) {
...
void ReferenceModel::setpointCallback(const geometry_msgs::Pose &setpoint_msg) {
  ...
  x_ref << setpoint_msg.position.x, setpoint_msg.position.y,
