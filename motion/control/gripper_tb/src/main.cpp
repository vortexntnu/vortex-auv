...
const double adc_to_duty = 820.0/1023.0; // (pwm_full_duty - pwm_zero_duty) / adc_resolution
...
void setup() {
  ...
  */
  ...
  /* dont start program unless dc control is at 0 and kill switch is not activated */ 
  check_duty_low();
  arm_esc();
}
// Main loop
void loop() {
  ...
  if (digitalRead(ks_pin)) { /* Kill switch not activated */
    ...
    /* dont continue program again unless dc control is at 0 and kill switch is not activated */ 
    check_duty_low();
  }
}
