    ...
    , pseudoinverse_allocator_(Eigen::MatrixXd::Zero(6, 8))
{
    ...
    thrust_update_period_ = std::chrono::milliseconds(static_cast<int>(
        1000 / get_parameter("propulsion.thrusters.thrust_update_rate").as_double()));
    ...
    pseudoinverse_allocator_.T_pinv = calculate_right_pseudoinverse(thrust_configuration_);
