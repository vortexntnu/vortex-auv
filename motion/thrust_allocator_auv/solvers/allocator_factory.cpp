...
std::unique_ptr<Allocator> Factory::make_allocator(const std::string& allocator_type, const AllocatorConfig& config){
    if (allocator_type == "pseudoinverse") {
      ...
      "Unknown allocator_type: '" + allocator_type + "'. Expected: pseudoinverse or qp.");
    }
};