...
void QPAllocator::formulate_as_qp(const AllocatorConfig& cfg) {
    ...
    // Save important constants from cfg that will be used in building CasADi
    // formulation
...
void QPAllocator::formulate_as_qp_CasADi() {
    ...
    lower_bound_constraint_matrix =
        casadi::DM::zeros(total_constraint_count, 1);
    upper_bound_constraint_matrix =
        casadi::DM::zeros(total_constraint_count, 1);

    lower_bound_state_matrix =
        casadi::DM::zeros(decision_variable_dimension, 1);
    upper_bound_state_matrix =
        casadi::DM::zeros(decision_variable_dimension, 1);
...
Eigen::VectorXd QPAllocator::calculate_allocated_thrust(
    const Eigen::VectorXd& tau) {
    if (!casadi_solver_initialized_) {
        throw std::runtime_error(
            "CasADi QP solver not initialized "
