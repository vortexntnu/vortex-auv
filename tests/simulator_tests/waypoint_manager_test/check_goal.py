...
pos_tol = 0.6  # meters (match python test tolerance)
ori_tol = 0.5  # rad (loose)
...
def main(args=None):
    ...
    print(f"Waiting for vehicle to reach goal: {goal_pos} with orientation {goal_ori}")

    start_time = time.time()
    timeout = 240  # seconds; waypoint manager may take longer

    x = y = z = None
    current_ori = (0.0, 0.0, 0.0)
    ...
    while rclpy.ok() and time.time() - start_time < timeout:
        ...
        if node.received_pose_:
            ...
            if dist < pos_tol and dist_ori < ori_tol:
                print(
                    f"Vehicle reached final waypoint: {goal_pos} and orientation: {goal_ori}"
                )
                print(f"Final vehicle pose: ({x, y, z}), {current_ori}")
                print(f"Euclidean error: {dist}, {dist_ori}")
                rclpy.shutdown()
                exit(0)

    print(
        f"Vehicle did not reach final waypoint: {goal_pos} and orientation: {goal_ori}"
    )
    if x is not None:
        print(f"Current_vehicle pose: ({x, y, z}), {current_ori}")
    print(
        f"Euclidean error: {dist if 'dist' in locals() else 'N/A'}, {dist_ori if 'dist_ori' in locals() else 'N/A'}"
    )
    rclpy.shutdown()
    exit(1)
